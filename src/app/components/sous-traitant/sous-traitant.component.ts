...
export class SousTraitantComponent implements OnInit {
 ...
 sousTraitantForm: FormGroup | any
  action = "ON_CREATE"
  msg = ''
  typeMsg = ''
      ...
      this._tiersService.update(formData).subscribe({
        next: (data: any) => {
          if (data.description == "success") {
            ...
            this.msg = "Le sous-traitant " + formData.numero + " " + formData.intitule + " a été modifié avec succès!"
            this.all()
      ...
      this._tiersService.create(formData).subscribe({
        next: (data: any) => {
          if (data.description == "success") {
            // executer avec success
            this.typeMsg = 'success'
            this.msg = "Le sous-traitant " + formData.numero + " " + formData.intitule + " a été enregistré avec succès!"
            this.all()
            this.resetsousTraitantForm()
          } else {
            // message en cas d'erreur
  ...
  onUpdate(formData: any) {
    this.sousTraitantForm.setValue(formData)
      ...
      this._tiersService.delete(formData.id).subscribe({
        next: (data: any) => {
          if (data.description == 'success') {
            ...
            this.typeMsg = 'success'
            this.msg = "Le sous-traitant " + formData.numero + " " + formData.intitule + " a été supprimé avec succès!"
  ...
  resetsousTraitantForm() {
    ...
    this.sousTraitantForm.controls['typeTiers'].setValue(this.typeTiers)
  }
  createForm() {
    this.sousTraitantForm = this._fbuilder.group({
      id: [null],
  ...
  openDialog(formData: any) {
    const dialogRef = this.dialog.open(ConfirmDialogComponent,
      {
        data: {
          message: 'Etes-vous sur de vouloir supprimer ce sous-traitant ?',
